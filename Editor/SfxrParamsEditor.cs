...
namespace usfxr
{
	...
	public class SfxrParamsEditor : PropertyDrawer
	{
		...
		void OnExpandedGUI(ref Rect position, SerializedProperty property)
		{
			...
			if (previewRect.width > 1) DoRenderPreview(SfxrPlayerGlobal.GetClip(PropertyToParams(property)), previewRect);
		...
		static void WidgetSlider(Rect position, SerializedProperty property)
		{
			...
			property.floatValue = EditorGUI.IntSlider(sliderPosition, label, Mathf.RoundToInt(property.floatValue * RangeScale), data.min, data.max) * RangeScaleToNormalized;
		...
		{
			SfxrPlayerGlobal.Play(PropertyToParams(property), true);
		}
